...
> 3. **`scripts/deploy.py --preserve-env` 已删除**。该 flag 静默吞 SSH 错误、曾经清空线上 .env。需要保留旧 env 的话，自己用 `/api/exec` 跑 `cat .env | base64`，解析后塞 `manifest.env` 全量传（snippet 见 §3c 末尾）。
...
> 6. **凭据优先级**：`manifest.env > vault export -p <project> > ~/.credentials/.env 抽前缀`。vault 不可用自动 fallback，task log 写一条 WARN，不阻塞部署。

> 📦 公开技能包（无需 token，含 SKILL.md + scripts/ + references/）：
>     `curl -o /tmp/skill.zip https://deploy.mvp.restry.cn/skill.zip && unzip -o /tmp/skill.zip -d ~/.hermes/skills/devops/mvp-deployer/`
>     一句话安装指令（给其他 agent 复制粘贴）：`curl https://deploy.mvp.restry.cn/install-instruction`
> 📊 Dashboard：`https://deploy.mvp.restry.cn/`（需 token）
>
...
**合并规则**：`finalEnv = { ...credEnv, ...manifestEnv }` —— **manifest 优先级更高**，能覆盖服务器侧同名 key。
...
| Skill 镜像 | `GET /skill.zip` / `GET /install-instruction` | 公开 |
...
### 3b-py. Python apps（FastAPI/uvicorn/Flask）⭐ 跟 node 不一样，先看这

**坑 1 — `type:python` 跳过 build phase**。manifest 里的 `build` 字段对 python 类型**不执行**（deployer 只对 node 跑 install/build）。venv 不会自动建。**正确姿势**：第一次 deploy 完（会因为缺 venv pm2 errored，没事），**手动**用 `/api/exec` 建 venv：

```bash
curl -sS -X POST https://deploy.mvp.restry.cn/api/exec/<project> \
  -H "Authorization: Bearer $MVP_DEPLOYER__TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"command":"python3 -m venv .venv && .venv/bin/pip install --no-cache-dir -r server/requirements.txt","timeoutMs":300000}'
```

然后 redeploy（manifest 里把 `.venv` 加进 `preserveData`），PM2 就能起。

**坑 2 — `run` 必须是 `.py` 文件**。`type:python` 时 deployer 在 ecosystem.config.js 强塞 `interpreter: <cwd>/.venv/bin/python3`，**不接受 shell 命令**。写 `"run": ".venv/bin/uvicorn server.app:app --host 0.0.0.0 --port 3813"` → PM2 把 `.venv/bin/uvicorn` 这个 ELF/shebang 二进制当 Python 源文件解析 → 崩 `SyntaxError: source code cannot contain null bytes`，restart 100k+。

**正确做法**：写个 `start.py` 入口让 python 跑：

```python
# start.py
import os, uvicorn
if __name__ == "__main__":
    uvicorn.run("server.app:app", host="0.0.0.0", port=int(os.environ.get("PORT", "3813")))
```

manifest:
```jsonc
{ "project": "skill", "type": "python", "port": 3813, "host": "skill.mvp.restry.cn",
  "run": "start.py",                                        // .py 文件，不是 shell 命令
  "preserveData": ["data", ".venv"],                        // .venv 必须保下来，否则 redeploy 又没了
  "env": {...},
  "smokePaths": ["/api/health"] }
```

**坑 3 — 老 deployer 实例 `manifest.host` 被忽略**。本文档 §0 的 v2 breaking changes 说"host 必填"，但**生产 deployer 可能还是 v1 实装**：route host 由项目名兜底拼 `<project>.mvp.restry.cn`，`manifest.host` 形同虚设。**deploy 完务必 `GET /api/status` 看 `route.hosts` 实际是什么**。要 host = `skill.mvp.restry.cn` 就把 `project` 命名为 `skill`，**不要叫 `skill-sync`**——名字不一样，route 就不一样。

> 排查信号：deploy response 里 `url` 字段不是你期望的 host = 中招。

**完整流程**（首次部署 Python app）：
1. zip 源码（含 `requirements.txt` + `start.py`）
2. `POST /api/deploy` → 文件就位但 PM2 大概率会 errored（venv 还没装）
3. `POST /api/exec/<p>` 跑 `python3 -m venv .venv && .venv/bin/pip install -r requirements.txt`
4. 再 `POST /api/deploy` 同样 manifest（这次带 `preserveData: [".venv"]`）→ PM2 重启成功
5. `GET /api/status` 确认 `pm2.status=online` + `route.hosts` 是你要的
6. curl `https://<host>/api/health` 验真活
...
```jsonc
{
  ...
  "preserveData": true,           // 默认 true；保留旧部署的 data/ 目录（不被 zip 覆盖）
...
| PM2 `SyntaxError: source code cannot contain null bytes` + `\x7fELF\x02\x01\x01` | `type:python` 配 shell 命令的 `run`，PM2 把 uvicorn/bash 二进制当 py 源码 | `run` 必须是 `.py` 文件，写 `start.py` wrapper（见 §3b-py 坑 2） |
| `type:python` deploy 完 venv 不存在 / `interpreter NOT AVAILABLE in PATH` | `manifest.build` 对 python 类型不执行 | 用 `/api/exec` 手动建 venv 后 redeploy（见 §3b-py 坑 1） |
| deploy response `url` 不是 manifest 里的 `host` | 老 deployer v1 实装忽略 `manifest.host`，按项目名兜底 | 把 `project` 改名等于想要的 subdomain（见 §3b-py 坑 3） |
...
11. **Python apps 的三大坑**（见 §3b-py）：(a) `manifest.build` 不执行，venv 手动用 `/api/exec` 建；(b) `run` 必须是 `.py` 文件不能是 shell 命令（否则 PM2 把 ELF 当 py 源码 → null bytes 崩）；(c) `manifest.host` 在老 deployer 上被忽略，**项目命名 = 想要的 subdomain**
...
| 路径 | 用途 |
|---|---|
| `scripts/deploy.py` | 异步部署 Python 脚本，轮询任务（v2: 已移除 --preserve-env，host 必填） |
