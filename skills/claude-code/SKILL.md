...
When dispatching CC and hitting recurring patterns (CC procrastinating, hallucinating fixes, ignoring credentials, etc.), grep `references/cc-playbook/` — 6 battle-tested prompt patterns:

- `cc-anti-procrastination-prompt.md` — defeat CC stalling/asking back/sub-delegating
- `cc-frontend-e2e-pitfalls.md` — browser-agent 4822 testing pitfalls
- `cc-prompt-explicit-export-commands.md` — give shell-ready `export X="$(vault get ...)"` lines, never prose
- `cc-prompt-grep-and-verify.md` — grep patterns + machine-verifiable acceptance > file lists + commit msgs
- `cc-review-verify-fix-loop.md` — review → independent verify → fix (avoid acting on static-analysis hallucinations)
- `claude-code-stepwise-refactor.md` — large multi-file refactor as numbered relay
...
| `/goal <condition>` | **Autonomous loop**: keep working until a fast judge model confirms the condition. See "Autonomous Loops" below. |
| `/loop [interval]` | **Autonomous loop**: re-trigger Claude every N seconds until you stop or Claude decides done. See "Autonomous Loops" below. |
